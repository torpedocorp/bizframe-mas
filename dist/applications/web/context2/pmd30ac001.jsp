 ...
 */
	...
	if (obid == null || obid.length() == 0) {
		json.put("err", "true");
		json.put("msg", _i18n.get("pmd30ac001.notfound"));
		out.print(json);
		return;
	}
	...
	pmode.setObid(obid);
	pmode.setFilePath(request.getParameter("filepath"));
	...
	if (mepbinding == Eb3Constants.PMODE_MEPBINDING_PUSH
	    ...
	    if(payloadProfName_arr != null) {
	        for (int i = 0; i <payloadProfName_arr.length; i++) {
	            ...
	            payload.setPropName(StringUtil.nullCheck(payloadProfName_arr[i]));
 ...
 engine.updateObject("PMode", obj, DAOFactory.EBMS3);
 json.put("err", "false");
 out.print(json);
 // remove
 PModeManager.removeCachePMode(id);
%>

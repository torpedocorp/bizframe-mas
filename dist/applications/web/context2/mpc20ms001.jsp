...
function getHeader() {
	...
	body += '  </COLGROUP>';
	body +='<tr>';
	body +='  <td class="ResultHeader"><%= _i18n.get("mpc20ms001.column.displayname") %></td>';
	body +='  <td class="ResultHeader"><%= _i18n.get("mpc20ms001.column.default") %></td>';
	body +='  <td class="ResultHeader"><%= _i18n.get("mpc20ms001.column.status") %></td>';
	body +='  <td class="ResultLastHeader"><%= _i18n.get("mpc20ms001.column.uri") %></td>';
	body +='</tr>';
	return body;
}
...
function showList(originalRequest) {
   ...
   for (var i=0; i<res.mpcs.length; i++) {
      ...
      body += '<tr>';
      body += '   <td class="ResultData" ><a href="mpc21ms001.jsp?obid='+res.mpcs[i].obid+'">' + res.mpcs[i].displayName + '</a></td>';
      ...
      body += '</tr>';
      if (res.mpcs[i].isDefaultVal == "1") {
      	body += '<tr><td colspan=4 class="ResultLastData"></td></tr>'
      }
   }
   if (res.totalRows == 0) {
      body += '<tr>';
        body += '<td colspan="4" class="ResultLastData" align="center"><%= _i18n.get("mpc20ms001.notfound")%></td></tr>';
   }
   body += "</table>";
   $('listContent').innerHTML = body;
...
function getMpcs() {
   ...
   var myAjax = new Ajax.Request("mpc20ac001.jsp", opt);
}
...
<table class="TableLayout">
  <tr>
    <td width="200" class="Title" ><img src="images/bu_tit.gif" ><%=_i18n.get("menu.mpc.localmpc.list")%></td>
...
<table class="SearchTable"  >
  <tr>
      <td style="padding:10px"><table>
        <tr>
          <td width="95"><img src="images/bu_search.gif" ><%= _i18n.get("mpc20ms001.column.displayname") %></td>
          <td width="250">
            <input type="text" name="displayName" id="displayName" class="FormText" size="32" onKeyDown="javascript:LoginEnterDown();" >
          </td>
          <td width="95"><img src="images/bu_search.gif" ><%= _i18n.get("mpc20ms001.column.status") %></td>
          <td width="250">
            <select name="status" id="status" style="width:187;" class='FormSelect'>
               <option value=""><%= _i18n.get("global.all") %></option>
                 <option value="<%= Eb3Constants.MPC_ACTIVE %>"><%= _i18n.get("mpc20ms001.str.active") %></option>
                 <option value="<%= Eb3Constants.MPC_INACTIVE %>"><%= _i18n.get("mpc20ms001.str.inactive") %></option>
            </select>
          </td>
          <td width="50" align="right">&nbsp;</td>
        </tr>
        <tr>
          <td width="95"><img src="images/bu_search.gif" ><%= _i18n.get("mpc20ms001.column.uri") %></td>
          <td width="250">
			<input type="text" name="uri" id="uri" class="FormText" size="32" onKeyDown="javascript:LoginEnterDown();" >
          </td>
          <td><img src="images/bu_search.gif" ><%= _i18n.get("mpc20ms001.column.desc") %></td>
...
<table class="TableLayout" >
  <tr> 
    <td align="right" style="padding-top:15">      
      <a href="mpc10ms001.jsp"><img src="images/btn_big_create.gif" width="39" height="23" border="0"></a>
    </td>
  </tr>
</table>

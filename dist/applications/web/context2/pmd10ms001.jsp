...
function insertPMode() {
	...
	if (mepbinding == "<%=Eb3Constants.PMODE_MEPBINDING_PULL%>"
		...
		if (document.form1.myrole[0].checked && $('mpc1uIsLocal').value =='1') {
			setMessage("LEG[1][u] <%=_i18n.get("pmd10ms001.remotempc.inform") %>");
		    return;
		}
		if (document.form1.myrole[1].checked && $('mpc1uIsLocal').value =='0') {
    ...
    if (mepbinding == "<%=Eb3Constants.PMODE_MEPBINDING_PUSHANDPULL%>") {
		...
		if (document.form1.myrole[0].checked && $('mpc2uIsLocal').value =='1') {
				setMessage("LEG[2][u] <%=_i18n.get("pmd10ms001.remotempc.inform") %>");
		        return;
		}
		if (document.form1.myrole[1].checked && $('mpc2uIsLocal').value =='0') {
	...
	var opt = {
	    ...
	    onSuccess: function(t) {
	    	...
	    	if (res.err == "true") {
				...
				Dialog.setInfoMessage('<%=_i18n.get("pmd10ms001.msg.registered") %>');
		    	timeout=2;
	...
	var myAjax = new Ajax.Request('./pmd10ac001.jsp', opt);
}
...
function uploadFileFunction(num) {
    var objPopup = window.open('','myWindow','resizable=no, top=0, left=0, width=200, height=100, scrollbars=no');
    document.form1.target="myWindow";      // 타켓
    document.form1.action = "pmd10ac002.jsp?num=" + num;
    if ( objPopup == null)                                  // 팝업이 뜨는지 확인
        alert('차단된 팝업창을 허용해 주세요');
     else{
        document.form1.submit();
        objPopup.focus();
	    if ($('schemaFilePath'+num).value == '') {
		   $('schemaFilePath'+num).value = '1';
	    }
      }
}

function deleteFileFunction(num) {
   var params = Form.serialize(document.form1);
   var opt = {
      method: 'post',
      parameters: params,
   ...
   var myAjax = new Ajax.Request("pmd10ac003.jsp?num=" + num, opt);
}
...
function addRowPayloadProfTbl(num)
{
  ...
  if (type.length > 0) {
      ...
      addRowPayloadProfTblLast(num);
  }
}

function addRowPayloadProfTblLast(num)
{
  var check_name = $('payloadProfName'+num).value;
  var type = $('payloadProfType'+num).value;
...
function delRowTbl(tblId, trnum, num) {
  ...
  if (tblId.indexOf("payloadProfTbl") > -1) {
	// 업로드 했던 파일 지우기
	deleteFileFunction(num);
  }
...
<table class="TableLayout" border=0>
  <tr>
    <td width="200" class="Title" ><img src="images/bu_tit.gif" ><%=_i18n.get("menu.pmd.register")%></td>
...
<table class="TableLayout" >
  <tr>
    <td align="right" style="padding-top:15">
      ...
      <a href="javascript:insertPMode();"><img src="images/btn_big_create.gif" width="39" height="23" border="0"></a>
    </td>
  </tr>
</table>
</form>

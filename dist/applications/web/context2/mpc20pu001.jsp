...
String fieldId = StringUtil.checkNull(request.getParameter("fieldId"));
...
function getHeader() {
	...
	body += '  <COL span="1" style="width:30;">';
	body += '  <COL span="1" style="width:30;">';
	body += '  <COL span="1" style="width:220;">';
	body += '  </COLGROUP>';
	body +='<tr>';
	body +='  <td class="ResultHeader"><%= _i18n.get("mpc20ms001.column.displayname") %></td>';
	body +='  <td class="ResultHeader"><%= _i18n.get("mpc20pu001.column.type") %></td>';	
...
function loading() {
   ...
   body = getHeader();
   body += '<tr>';
   body += '   <td colspan="5" class="ResultLastData" align="center"><%= _i18n.get("global.loading")%></td></tr>';
...
function showList(originalRequest) {
   ...
   for (var i=0; i<res.mpcs.length; i++) {
	  ...
	  body += '   <td class="ResultData" align=center>' + res.mpcs[i].type + '</td>';
      ...
      body += '   <input type="hidden" name="mpcIsLocal_'+i+'" value="'+res.mpcs[i].isLocal+'">';
      body += '   <input type="hidden" name="mpcUri_'+i+'" value="'+res.mpcs[i].uri+'">';
      body += '</tr>';
      if (res.mpcs[i].isDefaultVal == "1") {
      	body += '<tr><td colspan=5 class="ResultLastData"></td></tr>'
      }
   }
   if (res.totalRows == 0) {
      body += '<tr>';
        body += '<td colspan="5" class="ResultLastData" align="center"><%= _i18n.get("mpc20pu001.notfound")%></td></tr>';
   }
   body += "</table>";
   $('listContent').innerHTML = body;
...
function getMpcs() {
   ...
   var myAjax = new Ajax.Request("mpc20ac004.jsp", opt);
}
...
function applyMpc() {
	...
	if (sb_Index != "") {
	    ...
	    if ("<%=fieldId%>" != "") {
	        var isLocal = eval('document.form1.mpcIsLocal_'+sb_Index+'.value');
	        var mpcName = eval('document.form1.mpcName_'+sb_Index+'.value');
			opener.applyMpc("<%=fieldId%>", mpcObid, mpcUri, isLocal, mpcName);
			window.close();
	    } else {
			var mpcName = eval('document.form1.mpcName_'+sb_Index+'.value');
...
<table class="TableLayout">
  <tr>
    <td width="200" class="Title" ><img src="images/bu_tit.gif" ><%=_i18n.get("menu.mpc.select")%></td>

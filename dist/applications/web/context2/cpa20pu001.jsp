...
function getHeader() {
   ...
   body += '<table class="TableLayout" >';
	body += '  <COLGROUP>';
	body += '  <COL span="1" style="width:300;">';
   ...
   body += '      <td class="ResultLastHeader"><%=_i18n.get("global.cpa.lifetime")%></td>';
   body += '   </tr>';
	return body;
}
...
function showList(originalRequest) {
   ...
   for (var i=0; i<res.list.length; i++) {
      ...
      body += '	     <input type="radio" name="cpaIndex" id="cpaIndex_'+i+'" value="' + i + '">';
      body += '	     <a href="javascript:selectCpa(' + i + ')"><font class="blue-text03">' + res.list[i].name + '</font></a></td>';
      body += '   <td class="ResultData">'+res.list[i].partyName+'</td>';
      body += '   <td class="ResultLastData">'+res.list[i].start+ ' ~ ' + res.list[i].end+'</td>';
      body += '   <input type="hidden" name="cpaObid_'+i+'" value="'+res.list[i].obid+'">';
      body += '   <input type="hidden" name="cpaName_'+i+'" value="'+res.list[i].name+'">';
      body += '   <input type="hidden" name="cpaId_'+i+'" value="'+res.list[i].cpaId+'">';
      body += '</tr>';
   }
   if (res.totalRows == 0) {
      body += '<tr>';
        body += '<td colspan="3" class="ResultLastData" align="center"><%= _i18n.get("cpa20ms001.not.found")%></td></tr>';
   }
   body += "</table>";
   $('listContent').innerHTML = body;
...
function apply() {
	...
	if (sb_Index != "" && "<%=textId%>" != "") {
		var cpaObid = eval('document.form1.cpaObid_'+sb_Index+'.value');
		var cpaName = eval('document.form1.cpaName_'+sb_Index+'.value');
		var cpaId = eval('document.form1.cpaId_'+sb_Index+'.value');
		opener.applyRef(cpaObid, cpaName, cpaId);
		window.close();
	} else if (sb_Index != "") {
		var cpaObid = eval('document.form1.cpaObid_'+sb_Index+'.value');
		var cpaName = eval('document.form1.cpaName_'+sb_Index+'.value');
...
function selectCpa(index) {
	$('cpaIndex_' + index).checked = true;
}
...
<table class="TableLayout">
  <tr>
    <td width="200" class="Title" ><img src="images/bu_tit.gif" ><%=_i18n.get("menu.cpa.list")%></td>
  ...
  <tr>
      <td style="padding:10px" align="center"><table>
        <tr>
          ...
          <td width="200">
            <input type="text" name="cpaName" id="cpaName" class="FormText" size="32">
          </td>

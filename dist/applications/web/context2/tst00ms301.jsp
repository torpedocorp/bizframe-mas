...
function getMpc() {
	var params = 'actionObid=' + $('actionName').options[$('actionName').selectedIndex].value;
	var opt = {
		method: 'get',
		parameters: params,
		onSuccess: displayMpcList,
		on404: function(t) {
			$('messageDisplay').innerHTML = '<img src="images/bu_query.gif" align="absmiddle" ><%=_i18n.get("err00zz404.message") %>';
		//	showError('<%=_i18n.get("err00zz404.message") %>', null, null, null);
		},
	...
	var myAjax = new Ajax.Request("./tst00ac301.jsp", opt);
...
function displayMpcList(originalRequest) {
	...
	for (var i=0; i<res.mpcs.length; i++) {
		$('mpc_uri').options[i] = new Option(res.mpcs[i].displayName + "["
			+ res.mpcs[i].uri + "]" , res.mpcs[i].uri);
		if (i==0) {
			$('mpc_uri').options[i].selected = true;
		}
	}
}
...
<table class="FieldTable">
  ...
  <tr>
    ...
    <td colspan="3" class="FieldData">
      <select name="actionName" id="actionName" onChange="getDeliveryChannel(); getMpc();" class="FormSelect" style="width:220;"></select>
    </td>
  </tr>
  <tr>
  ...
  <tr>
  <td width="120" class="FieldLabel">MPC</td>
  <td colspan="3" class="FieldData">
    <select name="mpc_uri" id="mpc_uri" class="FormSelect" style="width:220;"></select>
  </td>
  </tr>
  <tr>
